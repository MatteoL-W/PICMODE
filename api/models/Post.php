...
class Post extends BaseModel
{
   public string $entity = 'post';
    ...
    {
        $this->db->query("SELECT COUNT(*) FROM post WHERE id = :id");
        $this->db->bind(':id', $idUser);
        return $this->db->fetchAll();
    }
    ...
    public function selectAllFollowedPostsFromLoggedUser (int $idUser){
        $this->db->query("SELECT post.*, user.pseudo FROM post INNER JOIN user ON post.idUser=user.id INNER JOIN following ON user.id=following.idFollowing ORDER BY date DESC");
        $this->db->bind();
        return $this->db->fetchAll();
    }
}
